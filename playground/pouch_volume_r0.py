...
# load image - Z, C, Y, X
pouch_img = io.imread(
    work_dir + 'vgQE-dsRed_mLamin-A488_eL3-L2 late sync_25C.lif - Series015.tif'
)

# reorder axes - C, Z, Y, X
#pouch_stack = pouch_img.swapaxes(0, 1)[0, 76:77:1, :, :]
pouch_stack = pouch_img[20:61:10, :, :, 0]

processing_steps = [
    ['EQU'],
    ['THR', 'OTSU', 70, 'no3D'],    # checked
    ['CLS', 'bin', 5],              # checked
    ['FILL'],
    ['OPN', 'bin', 2],              # checked
    ['CONV_BIT', 16, '3D'],
...
Plot.show_stacks(stack_fig, results, range(0, 6, 1), img_title=results_titles, colour_map=cmap)
